...
namespace BeghShare.Core.Services
{
    public class TcpTransportService : ISingleton, IAutoStart
    {
        ...
        private async Task AcceptLoop()
        {
            while (true)
            {
                try
                {
                    ...
                    var peerInfo = GetService<DiscoveryService>().GetPeerInfoByIpAddress(remoteEndPoint.Address);
                    if (peerInfo != null)
                    {
                    ...
                    {
                        GetService<DiscoveryService>().Discover(((IPEndPoint)client.Client.RemoteEndPoint!).Address.ToString());
                        client.Close();
                    }
                }
        ...
        private async Task ReceiveLoop(TcpClient client, IPEndPoint remoteEndPoint)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        _ = Task.Run(() => SendEvent(new TcpMsgReceivedEvent
                        {
                            Data = data,
        ...
        private static async Task<int> ReadExactlyAsync(Stream stream, byte[] buffer, int count)
        {
            ...
            while (total < count)
            {
                var read = await stream.ReadAsync(buffer, total, count - total);
                if (read == 0)
                    return total;
                total += read;
            }
